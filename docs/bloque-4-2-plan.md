...
## 0. Inconsistencias detectadas antes de proponer nada

Regla 19. Se reportan primero, sin resolverlas por cuenta propia.

### 0.1 La migración documental **no** está aplicada en Supabase

La instrucción de apertura da por hecho que el `main` reciente trae «migración
documental aplicada y validada en Supabase». El repositorio dice lo contrario,
de forma explícita, en `docs/migration-runbook-bloque-4.md:7`:

> **Esta migración NO se ha aplicado a ningún proyecto Supabase.** Se ha
> verificado únicamente contra un PostgreSQL 16 local y efímero.

Validada sí: contra PostgreSQL local y con 699 líneas de tests
(`tests/test_migrations_documents.py`). Aplicada a Supabase, no.

### 0.2 No existe adaptador PostgreSQL del repositorio documental

`DocumentRepositoryPort` solo tiene `memory_documents.py` (566 líneas). No hay
`postgres_documents.py`. El propio runbook lo declara en su §9.

Consecuencia dura: **no se pueden guardar vectores de chunks que no están en
PostgreSQL.** La mitad de persistencia del Bloque 4.2 depende de una pieza que
no existe, y construirla es un trabajo comparable al de este bloque
(`postgres_knowledge.py`, su equivalente del Bloque 2, tiene 1362 líneas).
...
### El bloque no cabe en una rama: se parte en dos

Regla de corte de la Skill. Con lo hallado en §0.2, el Bloque 4.2 completo
serían tres unidades de revisión, no una.

| Subbloque | Qué entrega | Depende de |
|---|---|---|
| **4.2.a — Elección del modelo** | Puerto corregido, adaptadores reales, banco de pruebas, corpus sintético, informe con mediciones y ADR de elección | Nada. Se puede empezar ya |
| **4.1.b — Adaptador PostgreSQL documental** | `postgres_documents.py` contra el esquema ya versionado | La migración del 4.1 (ya escrita) |
| **4.2.b — Persistencia vectorial** | Migración vectorial, registro de modelos, corridas, herramienta de embedding | 4.2.a (dimensión) y 4.1.b (dónde viven los chunks) |

**Propuesta de orden: 4.2.a ahora.** Es lo que la instrucción pide realmente
(elegir el modelo con evidencia), no toca base de datos, no toca Supabase, y
desbloquea todo lo demás. 4.1.b es un bloque propio que hay que agendar; 4.2.b
va después de los dos.

Lo que sigue detalla **4.2.a**, y delimita 4.2.b sin planificarlo al detalle.
...
4. **Al menos dos adaptadores reales** de los candidatos finalistas, en
   `src/elsa/adapters/`, elegidos por configuración. Dependencias en un grupo
   **opcional** de `pyproject.toml`: un clon limpio sigue pasando los tests sin
   instalarlas (regla 24).
...
|---|---|---|---|---|
| **D1 A** | Política de licencia para el modelo entregado a PAPELSA | (a) solo OSI permisiva (MIT/Apache-2.0); (b) se acepta *Gemma Terms of Use* con revisión legal | **(a)** para la elección por defecto, y medir EmbeddingGemma igualmente para tener el dato. Si (b) se aprueba, entra como candidato pleno | 0014 |
...
| **D4** | Tipo, métrica e índice | (a) `vector(N)` normalizado, coseno, **sin** índice ANN en el piloto; (b) HNSW desde el principio | **(a)**, ADR 0013 §6: el filtro de autorización no puede depender de un índice aproximado | 0013 |
...
| **D8 A** | Corte del bloque y orden | (a) 4.2.a → 4.1.b → 4.2.b; (b) un solo bloque grande | **(a)**, §1 | — |
...
| R3 | Un índice ANN con filtro de autorización devuelve menos resultados, o ninguno, en silencio | Búsqueda exacta en el piloto (ADR 0013 §6). Cuando haga falta índice, se mide el recall **con el filtro puesto** y con `hnsw.iterative_scan = strict_order`. La corrección nunca depende del índice |
...
|---|---|---|
| Reverificar cifras contra las tarjetas de los modelos | Pequeño | Acceso a red sin bloqueos |
...
| Dos o tres adaptadores reales | Medio | Descarga de pesos y runtime ONNX; la primera vez cuesta, la segunda no |
| Corridas de medición | Pequeño en tiempo de persona | Minutos de CPU por modelo sobre 200–400 chunks. No horas |
| Informe y ADR 0014 | Pequeño | Ya está el protocolo |
| **4.1.b** (prerrequisito de 4.2.b) | **Grande** | Reproducir en SQL las 566 líneas de semántica del adaptador en memoria, con sus tests de esquema |
| **4.2.b** | Medio | Migración, registro de modelos, corridas, herramienta |
...
- [`migration-runbook-bloque-4.md`](migration-runbook-bloque-4.md) — estado real de la migración documental
